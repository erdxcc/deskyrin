...
export function ensureUsersTable(database) {
    const exists = sqlRows(database
    ...
    const backupUsers = sqlRows(database.prepare(`SELECT * FROM users`).all());
    const hasQr = sqlRows(database
        .prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='qr_codes'`)
        .all()).length;
    const backupQr = hasQr
        ? sqlRows(database.prepare(`SELECT * FROM qr_codes`).all())
        : [];
    const hasEv = sqlRows(database
        .prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='scan_events'`)
        .all()).length;
    const backupEvents = hasEv
        ? sqlRows(database.prepare(`SELECT * FROM scan_events`).all())
        : [];
    ...
    database.exec(createUsersDdl());
    const insUser = database.prepare(`INSERT INTO users (
    ...
    for (const r of backupUsers) {
        ...
        const password_hash = bcrypt.hashSync(`MIGRATED_${randomBytes(16).toString("hex")}`, 12);
        insUser.run(r.id, email, password_hash, r.created_at, r.updated_at, r.last_scan_at, r.external_ref, r.wallet_public_key, r.wallet_secret_encrypted, r.created_at);
    }
    database.exec(createQrCodesDdl());
    const insQr = database.prepare(`INSERT INTO qr_codes (
    ...
    for (const q of backupQr) {
        insQr.run(q.bottle_id, q.partner_id, q.status, q.product_name, q.reward_usdc_micro, q.metadata_uri, q.assigned_user_id, q.bottle_mint_pubkey, q.mint_tx_signature, q.recycle_tx_signature, q.created_at, q.updated_at);
    }
    database.exec(createScanEventsDdl());
    const insEv = database.prepare(`INSERT INTO scan_events (id, created_at, bottle_id, user_id, event_type, payload_json, client_fingerprint)
     VALUES (?, ?, ?, ?, ?, ?, ?)`);
    for (const e of backupEvents) {
        insEv.run(e.id, e.created_at, e.bottle_id, e.user_id, e.event_type, e.payload_json, e.client_fingerprint);
    }
