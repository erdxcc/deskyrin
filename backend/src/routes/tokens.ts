...
tokensRouter.get("/stake-options", (_req, res) => {
  res.json({
    ...
    curve:
      "Total PT at maturity scales with lock length (7d→1.10× … 90d→2.75× AC). Vesting is linear over the lock period.",
  });
});

const stakeBody = z.object({
  amountAc: z.number().int().positive(),
  lockDays: z.number().int().refine(
    (n) => [7, 14, 30, 60, 90].includes(n),
    "Use 7, 14, 30, 60, or 90"
  ),
});
...
tokensRouter.post("/stake", requireAuth, (req, res, next) => {
  ...
  try {
    const out = stakingService.createStake(
      req.authUser!.id,
      parsed.data.amountAc,
      parsed.data.lockDays
    );
...
tokensRouter.post("/stakes/:stakeId/claim", requireAuth, (req, res, next) => {
  try {
    const out = stakingService.claimStake(
      req.authUser!.id,
      req.params.stakeId
    );
    res.json(out);
