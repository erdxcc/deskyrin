...
import { getConnection, getPartnerKeypair, programIdPk } from "./recyclingChain.js";
...
export function deskyrinConfigPda(): PublicKey {
...
export function stakePositionPda(user: PublicKey, stakeIdx: bigint): PublicKey {
  const idxBuf = Buffer.alloc(8);
  idxBuf.writeBigUInt64LE(stakeIdx, 0);
  const [pda] = PublicKey.findProgramAddressSync(
    [STAKE_POS_SEED, user.toBuffer(), idxBuf],
    programIdPk()
  );
  return pda;
}
...
export function programAuthorityPda(): PublicKey {
  const [pda] = PublicKey.findProgramAddressSync(
