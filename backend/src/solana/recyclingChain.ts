...
let partnerKeypairCache: Keypair | null = null;
...
export function getPartnerKeypair(): Keypair {
  if (partnerKeypairCache) return partnerKeypairCache;
  const raw = config.solanaPartnerKeypairJson.trim();
  if (!raw) {
    const err = new Error("SOLANA_PARTNER_KEYPAIR missing");
    (err as Error & { code: string }).code = "SOLANA_NOT_CONFIGURED";
    throw err;
  }
...
export async function mintBottleOnChain(input: {
...
}): Promise<{ bottleMint: Keypair; signature: string }> {
  ...
  const [poolState] = PublicKey.findProgramAddressSync(
    [POOL_STATE_SEED, partner.publicKey.toBuffer()],
    programId
  );
  const [bottleRecord] = PublicKey.findProgramAddressSync(
    [BOTTLE_SEED, Buffer.from(input.bottleId, "utf8")],
    programId
  );
  const [bottleEscrow] = PublicKey.findProgramAddressSync(
    [BOTTLE_ESCROW_SEED, bottleMint.publicKey.toBuffer()],
    programId
  );
  const escrowBottleAta = getAssociatedTokenAddressSync(
...
export async function recycleBottleOnChain(input: {
  ...
  const [bottleEscrow] = PublicKey.findProgramAddressSync(
    [BOTTLE_ESCROW_SEED, input.bottleMintPubkey.toBuffer()],
    programId
  );
  const escrowBottleAta = getAssociatedTokenAddressSync(
    input.bottleMintPubkey,
    bottleEscrow,
    true
  );
  const userUsdcAta = getAssociatedTokenAddressSync(
    usdcMint,
    input.custodialUserPubkey,
    false
  );
  const rewardPoolUsdc = getAssociatedTokenAddressSync(
    usdcMint,
    programAuthority,
    true
  );
