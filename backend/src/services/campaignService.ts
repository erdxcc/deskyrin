...
import type { CampaignRow, CampaignTaskRow } from "../types.js";
...
function taskRowToPublic(
  row: CampaignTaskRow,
  progress: { progress_count: number; completed: number } | null
): PublicCampaignTask {
...
export function listCampaigns(): PublicCampaignSummary[] {
  const database = getDb();
  const rows = sqlRows<
    CampaignRow & { partner_name: string; task_count: number }
  >(
    database
      .prepare(
...
export function getCampaignDetail(
  campaignId: string,
  userId: string | null
): PublicCampaignDetail | null {
  const database = getDb();
  const c = sqlRow<CampaignRow>(
    database.prepare(`SELECT * FROM campaigns WHERE id = ?`).get(campaignId)
  );
  if (!c) return null;
  const partner = partnerService.getPartner(c.partner_id);
  if (!partner) return null;

  const tasks = sqlRows<CampaignTaskRow>(
    database
      .prepare(
        `SELECT * FROM campaign_tasks WHERE campaign_id = ? ORDER BY sort_order ASC, created_at ASC`
      )
      .all(campaignId)
  );
  ...
  let progressMap = new Map<
    string,
    { progress_count: number; completed: number }
  >();
  if (userId) {
    const prog = sqlRows<{
      task_id: string;
      ...
      database
        .prepare(
          `SELECT task_id, progress_count, completed FROM user_task_progress
  ...
  return {
    ...
    tasks: tasks.map((t) =>
      taskRowToPublic(t, progressMap.get(t.id) ?? null)
    ),
  };
}
...
export function seedCampaignIfMissing(input: {
  ...
  const database = getDb();
  const exists = sqlRow<{ n: number }>(
    database.prepare(`SELECT 1 AS n FROM campaigns WHERE id = ?`).get(input.id)
  );
  if (exists) return;

  database
    .prepare(
      `INSERT INTO campaigns (id, partner_id, title, description, influencer_name, partner_ad_note)
       VALUES (?, ?, ?, ?, ?, ?)`
    )
    .run(
      input.id,
  ...
  const insTask = database.prepare(
    `INSERT INTO campaign_tasks (id, campaign_id, title, description, target_count, token_reward, sort_order)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  );
...
export function recordTaskStep(
  userId: string,
  taskId: string
): { user: userService.PublicUser; campaign: PublicCampaignDetail; awardedAc: number } {
  const database = getDb();
  const task = sqlRow<CampaignTaskRow>(
    database.prepare(`SELECT * FROM campaign_tasks WHERE id = ?`).get(taskId)
  );
  if (!task) {
  ...
  try {
    const cur = sqlRow<{
      ...
      database
        .prepare(
          `SELECT progress_count, completed FROM user_task_progress WHERE user_id = ? AND task_id = ?`
        )
        .get(userId, taskId)
    );
    ...
    if (cur && cur.completed === 1) {
      ...
      (err as Error & { code: string }).code = "TASK_ALREADY_COMPLETED";
      throw err;
    }
    ...
    if (next >= task.target_count) {
      ...
      database
        .prepare(
          `INSERT INTO user_task_progress (user_id, task_id, progress_count, completed, updated_at)
  ...
  if (!campaign) {
    ...
    (err as Error & { code: string }).code = "CAMPAIGN_NOT_FOUND";
    throw err;
  }
